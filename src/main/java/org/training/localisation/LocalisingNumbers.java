...
public class LocalisingNumbers {
    ...
    public static void formatNumbers(){
        ...
        System.out.println(NumberFormat.getNumberInstance(Locale.FRANCE).format(n));// 77000,11
    }
    ...
    public static void parseNumbers(){
        // parse: String --> number
        String unitedStatesNumber = "77,000.11", italianNumber = "77.000,11", frenchNumber = "77000,11";
        try {
    ...
    public static void mexicanCurrency(){
        // Crear Locale para México
        Locale mexicoLocale = new Locale("es", "MX");

        // Obtener la moneda para México
        //Currency currency = Currency.getInstance(mexicoLocale);

        // Crear formateador de números para moneda
        NumberFormat currencyFormatter = NumberFormat.getCurrencyInstance(mexicoLocale);

        // Valor numérico a formatear
        double amount = 1234.56;

        // Formatear el valor numérico como moneda mexicana
        String formattedAmount = currencyFormatter.format(amount);

        // Mostrar el resultado
        System.out.println("Cantidad formateada en moneda mexicana: " + formattedAmount);
    }
}