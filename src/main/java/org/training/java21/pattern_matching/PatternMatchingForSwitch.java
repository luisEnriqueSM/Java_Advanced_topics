    ...
    public static void patternMatchingSwitch(Vehicle v){
        System.out.println(
                switch (v){ // 'v' is the "selector expression"
                    // 'Boot b' is a (type) pattern label
                    ...
                    case Vehicle vehicle -> "Generic Vehicle"; // uncoditional pattern and "default" not allowed as:
                    //default -> "Invalid type"; // - dominated/unreachable if "Vehicle vehicle" type pattern there
                    case null -> "Invalid type"; // dominated/unreachable if "default" type pattern there
                    //case null, default -> "Invalid type"; // java: switch has both an unconditional pattern and a default label
                }
        );
    }
